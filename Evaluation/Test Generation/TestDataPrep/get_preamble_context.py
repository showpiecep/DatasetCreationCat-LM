...
def extract_code_before_first_test(java_code):
    tree = parser.parse(bytes(java_code, 'utf8'))
    first_test_method = find_first_test_method(tree)
    if first_test_method:
        start_line = first_test_method.start_point[0]
...
if __name__ == '__main__':
    ...
    code_preamble = extract_code_before_first_test(java_code)
    with open(output_file, 'w') as f:
        f.write(code_preamble+"\n")